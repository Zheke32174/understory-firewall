...
| InviZible Pro | `tor` ELF | Per-ABI prebuilt binary InviZible ships in `assets/`, **renamed `.mp3`** so Android's packager will not compress or extract it | `tools/donor-assets/fetch.sh tor-android` — builds from upstream source (dist.torproject.org) against a cross zlib/OpenSSL/libevent, four blockers documented in the script | **BUILDABLE** — produced a ~20 MB static aarch64 Android ELF here (no INTERP/dynamic section, so no ELF repatching, runs under any uid); not committed, not device-tested |
| InviZible Pro | `i2pd` ELF | Per-ABI prebuilt binary InviZible ships in `assets/`, **renamed `.mp3`** so Android's packager will not compress or extract it | `tools/donor-assets/fetch.sh i2pd-android` — builds `PurpleI2P/i2pd` against a cross Boost + the shared OpenSSL/zlib prefix, three blockers documented in the script | **BUILDABLE** — produced a ~17.5 MB stripped static aarch64 Android ELF here (`main` exported, ~53k i2p symbols, no dynamic section); not committed, not device-tested |
