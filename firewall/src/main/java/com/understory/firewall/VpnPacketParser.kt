...
/**
 * Minimal IPv4 + UDP packet parser/builder for the firewall's tun
 * reader. Scope is intentionally narrow: we only need to identify
 * UDP DNS queries so we can redirect them to the local DNSCrypt
 * proxy. Full TCP stack support is phase 3b (deferred).
 *
 * Wire layout we parse:
 *
 ...
 *     6-7 Checksum
 *
 * IPv6 (RFC 8200): not parsed in this commit. Tun-level DNS on IPv6
 * is rare in practice — most app DNS goes via IPv4 even on dual-stack
 * networks because the system resolver picks v4 first. Phase 3b can
 * add v6 if needed.
 */
object VpnPacketParser {
    ...
    /** Parsed view of an IPv4 + L4 packet. Lengths refer to byte
     *  positions in the original buffer (offset relative to the start
     *  of the buffer the packet was read into). */
    data class Ipv4Udp(
    ...
    /** Parse an IPv4 + UDP packet. Returns null if the buffer isn't
     *  v4, isn't UDP, or is malformed (truncated header, bad lengths). */
    ...
    /** TCP destination port at the canonical offset. Returns null if
     *  the packet isn't IPv4 TCP or is truncated. Used by phase-2's
     *  port-block path before forwarding decisions; full TCP parsing
     *  is phase 3b. */
    ...
    /**
     * Build an IPv4+UDP response packet by swapping src/dst IP and
     * port from a request packet, replacing the UDP payload with the
     * supplied bytes, and recomputing the UDP and IP checksums.
     *
     * Used to wrap a DNS response from the local proxy back into a
     * tun-writable packet that the original querying app sees as
     * coming from the IP it queried.
     */
    fun buildIpv4UdpResponse(
        request: Ipv4Udp,
        responsePayload: ByteArray,
    ): ByteArray {
        val ipHeaderLen = 20  // we don't preserve options on the response
        val udpHeaderLen = 8
        ...
        // Identification: 0 — fine for UDP, no fragmentation expected.
        out[4] = 0; out[5] = 0
        ...
        // UDP checksum: optional in IPv4 but most stacks set it.
        // We compute the proper pseudo-header + UDP-header + payload checksum.
    ...
    private fun udpChecksum(
        ...
        sum += udpLen
        // UDP header + payload, two bytes at a time. Last byte padded with 0
        // if odd length.
        var i = udpStart
