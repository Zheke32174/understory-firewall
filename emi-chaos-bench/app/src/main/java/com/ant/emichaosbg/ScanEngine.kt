...
/**
 * The page's window onto the native engine. READ-ONLY on purpose: there is no method here that
 * lets the WebView raise, edit, suppress or delete a finding. It can start and stop scanning
 * and it can look — that is the whole surface. The detection and the recording happen on the
 * other side of this boundary, which is the point of moving them.
 */
class ScanBridge(ctx: Context, private val engine: ScanEngine) {
    ...
    @android.webkit.JavascriptInterface
    fun networks(): String = engine.lastWifiJson()
}
