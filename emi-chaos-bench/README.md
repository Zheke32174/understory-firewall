...
With Bluetooth switched off, running the BLE chaotic cadence produced an audible stutter in
the sound and a hitch in the app. That was desirable and losing it was my error.

*What actually caused it:* `EMIBridge.bleScan()` is a **synchronous** JS-to-native bridge call,
and the audio engine is a `ScriptProcessorNode` — which also runs on the **main thread**. With
Bluetooth off the native call returns instantly (null LE scanner), so the cadence fired far
more often and hammered the main thread with bridge round-trips and DOM writes. Those collide
with the audio callback's real-time window and cause buffer underruns. With Bluetooth on,
Android's BLE scan limiter slowed the cadence, collisions became rare, and the effect vanished.

It is now a standalone **Scan stutter** control that reproduces the *same mechanism* rather
than imitating the sound — real synchronous bridge bursts plus DOM churn at randomised
intervals — but using a **Bluetooth-independent** call, so it behaves identically with the
adapter on or off. The audible glitch (short holds and drops, exactly what a stalled or
dropped buffer sounds like) is also asserted directly in the audio path so it doesn't depend
on how fast a given phone's bridge is. Bounded: the burst is capped and the main thread is
never held long enough to risk an ANR. Measured at heavy severity: ~22 bursts/s, 1212 real
bridge calls over 6s, audio surviving and recovering cleanly.

**BLE cadence is no longer governed at all.** 3.7 moved `Governor.evaluate()` into the display
loop so it ran constantly instead of only during audio — which let it step up and start
de-rating the *BLE* cadence's timings, something it never did in BadJack. That was my
regression damping the behaviour above. The governor now applies only to Wi-Fi and the sensor
blaster, where the platform's scan budget and thermal load are genuinely at stake. Verified:
with the governor pinned to maximum de-rate, the BLE mode distribution is statistically
identical to nominal while Wi-Fi's marathon share correctly falls 12.8% → 2.0%.


**App died when swiped from recents (reported).** Two separate causes. The service wasn't
marked `stopWithTask="false"`, so the OS took it down with the task — but fixing that alone
would have been worse than useless, because the sound is generated by the **WebView inside the
Activity**, which task removal destroys regardless. A surviving service with no WebView is a
notification claiming to mask while producing silence. So `onTaskRemoved` now stands up a
**headless WebView inside the service**, loads the same page and starts it; if that can't be
created, the service stops rather than lying.

**Mic: native fallback, because the OS can capture even when WebView won't.** Your diagnostic
settled it — `nativeCaptureOk: true`, `recordingState: 3` (actually recording), zero other
recorders, mic unmuted, in the same process at the same moment `getUserMedia` returned
`NotReadableError`. Two changes:

- **`MODIFY_AUDIO_SETTINGS` is now declared.** Chromium routes mic capture through the same
  audio stack WebRTC uses, which calls into `AudioManager` while opening an input stream.
  Without that permission those calls fail and the stream never opens — surfacing as exactly
  the bare "Could not start audio source" you saw. A plain `AudioRecord` doesn't need it,
  which is precisely why the native probe succeeded while WebView failed. It's a normal
  install-time permission, no prompt. This is the most likely single fix.
- **A full native analyser (`NativeMic.kt`) as fallback**, because relying on that alone would
  leave the feature one WebView quirk from breaking again. `AudioRecord` at 48kHz → 2048-point
  FFT → speech-band RMS, ultrasonic energy, and a 128-bin spectrum, pushed over the bridge and
  driving the identical UI. Same privacy invariant, enforced by construction: samples are
  analysed in the capture loop and discarded in the same iteration — only derived numbers ever
  leave the class.

**Two more confirmed by adversarial review — including one where my own previous "fix" didn't
work:**

- **The infrasound ring was still 8× off.** 3.7 changed `feedLong` to count samples, but it's
  called once per 2048-sample buffer, and since 2048 ≥ 256 it wrote exactly one entry per call
  and reset — so the true rate was sr/2048 = 23.4Hz while `longRate()` reported 187.5Hz. Every
  infrasound reading was mislabelled by 8×, the top of the band sat above the ring's real
  Nyquist unpoliced, and the warm-up gate took 5.5s instead of 0.7s. A drain loop would *not*
  have fixed it either — there was only one value per buffer, so emitting it repeatedly would
  fabricate resolution never captured. The envelope is now accumulated **per sample** alongside
  the low-band ring. Verified: 187 entries per second of audio against a claimed 187.5.
- **Avocado stranded on a dead buffer slot.** Shrinking the "Buffers" slider mid-run left
  playback reading a slot recording no longer targets; the only escape was a per-wrap re-draw
  gated on Repeat probability, which at its maximum of 99 is a 1% chance per wrap — tens of
  seconds of stale, decaying audio. Playback index is now clamped. Deliberately *not* the
  record index: an in-flight recording would have its tail spliced into slot 0, corrupting a
  slot actively being written.
