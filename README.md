...
**Godwall** — an offline egress dashboard for a Tailscale user. In the default Companion mode it observes, explains, and one-tap-routes you to the network controls you already have (tunnel posture, Android's per-app data restrictions, system Private DNS, and which apps hold remote-admin power) — **it never takes the VPN slot and blocks nothing**. Real per-app packet blocking exists only in an opt-in, default-off **Standalone mode** that refuses to start whenever any other VPN is present.

The Standalone **DNS-filter tunnel** does encrypted, filtered DNS natively (no bundled resolver daemon): verified DoT / DoH, and **native DNSCrypt v2** (Ed25519 cert verification + X25519 + XSalsa20/XChaCha20-Poly1305, gated behind a runtime crypto self-test) using the canonical dnscrypt-proxy resolver lists bundled offline. DNS can additionally be routed **over Tor (Orbot) or I2P** via their SOCKS proxy, and the tun's packets can be **captured to standard `.pcap`** for Wireshark/PCAPdroid. System-level DNS hardening is still DoT via the OS's own Private DNS; egress canaries prove on the wire what your DNS/exit actually is. (Overlay routing remains out; see `docs/design-v2/firewall.md` and `docs/DONOR-ABSORPTION.md`.)
